...
echo ==========================================================
echo  [5/5] 🛡️ Checking Windows Defender Firewall (Ports 8005 & 8006)...
echo ==========================================================
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { $r = Get-NetFirewallRule -DisplayName 'LuminoAI Server (Port 8005)' -ErrorAction SilentlyContinue; if (!$r) { New-NetFirewallRule -DisplayName 'LuminoAI Server (Port 8005)' -Direction Inbound -LocalPort 8005 -Protocol TCP -Action Allow -ErrorAction Stop | Out-Null; Write-Host ' [OK] Created Inbound Windows Firewall rule for Port 8005.' -ForegroundColor Green } else { Write-Host ' [OK] Inbound Windows Firewall rule for Port 8005 is ACTIVE.' -ForegroundColor Green } } catch { Write-Host ' [NOTE] Could not auto-add firewall rule. If phones cannot connect, open Port 8005 in Windows Firewall.' -ForegroundColor Yellow }"
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { $r = Get-NetFirewallRule -DisplayName 'LuminoAI Rework HTTPS (Port 8006)' -ErrorAction SilentlyContinue; if (!$r) { New-NetFirewallRule -DisplayName 'LuminoAI Rework HTTPS (Port 8006)' -Direction Inbound -LocalPort 8006 -Protocol TCP -Action Allow -ErrorAction Stop | Out-Null; Write-Host ' [OK] Created Inbound Windows Firewall rule for Port 8006 (HTTPS Live Video).' -ForegroundColor Green } else { Write-Host ' [OK] Inbound Windows Firewall rule for Port 8006 is ACTIVE.' -ForegroundColor Green } } catch { Write-Host ' [NOTE] If mobile camera cannot connect, open Port 8006 in Windows Firewall.' -ForegroundColor Yellow }"

:: Automatically free ports 8005 and 8006 if occupied by orphaned processes
...
echo ==========================================================
echo  📡 Active Network Adapters ^& Rework Station Listeners:
echo ==========================================================
powershell -NoProfile -ExecutionPolicy Bypass -Command "$found=0; Get-NetIPConfiguration | Where-Object { $_.IPv4Address -ne $null } | ForEach-Object { $alias = $_.InterfaceAlias; $ip = $_.IPv4Address.IPAddress; $gw = $_.IPv4DefaultGateway.NextHop; $adapter = Get-NetAdapter -Name $alias -ErrorAction SilentlyContinue; if ($adapter.Status -eq 'Up' -and $ip -notmatch '^169\.254\.') { $found=1; if ($gw) { $type = 'Factory LAN / Wi-Fi [Router Connected]' } else { $type = 'Direct / Machine Camera Network [Isolated]' }; Write-Host ('   • ' + $alias + ' (' + $type + ')') -ForegroundColor Cyan; Write-Host ('     IP Address:   ' + $ip) -ForegroundColor White; Write-Host ('     ⚡ Live Video Scanner (Mobile HTTPS): https://' + $ip + ':8006/rework') -ForegroundColor Green; Write-Host ('     🌐 Standard Station (HTTP):           http://' + $ip + ':%PORT%/rework') -ForegroundColor DarkGray } }; if ($found -eq 0) { Write-Host '   ⚠️ No connected LAN adapter found. Connect PC to Wi-Fi or Ethernet.' -ForegroundColor Yellow }"
echo ==========================================================
